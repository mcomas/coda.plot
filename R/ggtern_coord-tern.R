...
CoordTern <- ggproto("CoordTern", CoordCartesian,
                     ...
                     render_bg       = function(self, panel_params, theme){
                       ...
                       grid::gTree(
                         children = do.call(grid::gList,items)
                       )
                     },
...
.render.ticks  <- function(self,data.extreme,panel_params,theme,items,X=self$required_scales){
  ...
  for(x in X){
    ...
    # })
    df = rbind(
      .get.grid.data(self,theme,data.extreme,X = x, major = T, angle = angle[ix], angle.text = angle.text[ix]),
      .get.grid.data(self,theme,data.extreme,X = x, major = F, angle = angle[ix], angle.text = angle.text[ix])
    )
...
.render.grid <- function(self,data.extreme,panel_params,theme,items,X=self$required_scales){
  ...
  ##get the base data.
  df = do.call('rbind', c(
    lapply(seq_along(seq.tlr), function(ix){
      cbind('major' = T, 'ix' = ix,
        .get.grid.data(self,theme,data.extreme,X = seq.tlr[ix], major = T, angle = angle[ix], angle.text = angle.text[ix]))
    }),
    lapply(seq_along(seq.tlr), function(ix){
      cbind('major' = F, 'ix' = ix,
        .get.grid.data(self,theme,data.extreme,X = seq.tlr[ix], major = F, angle = angle[ix], angle.text = angle.text[ix]))
    })))
  # print(df)
